...
namespace qpl {
	std::array<char, qpl::detail::info_header_size> qpl::detail::create_bitmap_info_header(qpl::size width, qpl::size height) {
		...
		info_header[14] = (char)(qpl::pixel_rgb::bytes() * 8);
	...
	std::array<char, qpl::detail::file_header_size> qpl::detail::create_bitmap_file_header(qpl::size width, qpl::size height, qpl::size padding_size) {
		auto file_size = static_cast<int>(qpl::detail::file_header_size + qpl::detail::info_header_size + (qpl::pixel_rgb::bytes() * width + padding_size) * height);
	...
	void qpl::generate_bmp(const std::vector<qpl::pixel_rgb>& image, qpl::size width, qpl::size height, const std::string& filename) {
		...
		auto file_header = qpl::detail::create_bitmap_file_header(width, height, padding_size);
		auto info_header = qpl::detail::create_bitmap_info_header(width, height);
