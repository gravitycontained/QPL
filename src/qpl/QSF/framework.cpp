...
namespace qsf {
	...
	void qsf::framework2::display() {
		this->window.display();
	}
	void qsf::framework2::internal_update() {
	...
	bool qsf::framework2::game_loop_segment() {
		if (!this->is_created()) {
			this->create();
		}
	...
	void qsf::framework2::set_framerate_limit(qpl::u32 value) {
		this->m_framerate_limit = value;
	}
	qpl::u32 qsf::framework2::get_framerate_limit() const {
		return this->m_framerate_limit;
	}
	void qsf::framework2::disable_framerate_limit() {
		this->m_framerate_limit = 0u;
	}
	void qsf::framework2::enable_update_if_no_focus() {
		this->m_update_if_no_focus = true;
	}
	void qsf::framework2::disable_update_if_no_focus() {
		this->m_update_if_no_focus = false;
	}
	bool qsf::framework2::is_update_if_no_focus_enabled() const {
		return this->m_update_if_no_focus;
	}
	bool qsf::framework2::has_focus() const {
		return this->m_focus;
	}
	bool qsf::framework2::has_gained_focus() const {
		return this->m_gained_focus;
	}
	bool qsf::framework2::has_lost_focus() const {
		return this->m_lost_focus;
	}
	qpl::time qsf::framework2::no_focus_time() const {
	...
	qpl::time qsf::framework2::run_time() const {
		return this->m_run_time_clock.elapsed();
	}
	qpl::time qsf::framework2::frame_time() const {
		return this->m_frametime;
	}
	void qsf::framework2::add_render(const std::string& name, bool smooth) {
	...
	qsf::render_texture& qsf::framework2::get_render(const std::string& name) {
	...
	const qsf::render_texture& qsf::framework2::get_render(const std::string& name) const {
		if (this->m_render_textures.find(name) == this->m_render_textures.cend()) {
			throw std::runtime_error(qpl::to_string("couldn't find render with name \"", name, "\""));
		}
	...
	void qsf::framework2::play_sound(const std::string& name, qpl::f32 volume, qpl::f32 speed) {
		qsf::play_sound(name, volume, speed);
	}
	void qsf::framework2::add_font(const std::string& name, const std::string& path) {
		qsf::add_font(name, path);
	}
	void qsf::framework2::add_sound(const std::string& name, const std::string& path) {
		qsf::add_sound(name, path);
	}
	void qsf::framework2::add_texture(const std::string& name, const std::string& path) {
		qsf::add_texture(name, path);
	}
	void qsf::framework2::add_sprite(const std::string& name, const std::string& path) {
		qsf::add_sprite(name, path);
	}
	void qsf::framework2::add_sprite(const std::string& name, sf::Texture& texture) {
		qsf::add_sprite(name, texture);
	}
	void qsf::framework2::add_shader(const std::string& name, const std::string& path, sf::Shader::Type shader_type) {
		qsf::add_shader(name, path, shader_type);
	}
	void qsf::framework2::add_shader(const std::string& name, const std::string& path) {
		qsf::add_shader(name, path);
	}
	void qsf::framework2::add_text(const std::string& name) {
		qsf::add_text(name);
	}


	void qsf::framework2::add_font_from_memory(const std::string& name, const std::string& memory) {
		qsf::add_font_from_memory(name, memory);
	}
	void qsf::framework2::add_sound_from_memory(const std::string& name, const std::string& memory) {
		qsf::add_sound_from_memory(name, memory);
	}
	void qsf::framework2::add_texture_from_memory(const std::string& name, const std::string& memory) {
		qsf::add_texture_from_memory(name, memory);
	}
	void qsf::framework2::add_shader_from_memory(const std::string& name, const std::string& memory, sf::Shader::Type shader_type) {
		qsf::add_shader_from_memory(name, memory, shader_type);
	}


	sf::Font& qsf::framework2::get_font(const std::string& name) {
		return qsf::get_font(name);
	}
	sf::SoundBuffer& qsf::framework2::get_sound(const std::string& name) {
		return qsf::get_sound(name);
	}
	sf::Texture& qsf::framework2::get_texture(const std::string& name) {
		return qsf::get_texture(name);
	}
	sf::Sprite& qsf::framework2::get_sprite(const std::string& name) {
		return qsf::get_sprite(name);
	}
	sf::Shader& qsf::framework2::get_shader(const std::string& name) {
		return qsf::get_shader(name);
	}
	qsf::text& qsf::framework2::get_text(const std::string& name) {
		return qsf::get_text(name);
	}

	const sf::Font& qsf::framework2::get_font(const std::string& name) const {
		return qsf::get_font(name);
	}
	const sf::SoundBuffer& qsf::framework2::get_sound(const std::string& name) const {
		return qsf::get_sound(name);
	}
	const sf::Texture& qsf::framework2::get_texture(const std::string& name) const {
		return qsf::get_texture(name);
	}
	const sf::Sprite& qsf::framework2::get_sprite(const std::string& name) const {
		return qsf::get_sprite(name);
	}
	const sf::Shader& qsf::framework2::get_shader(const std::string& name) const {
		return qsf::get_shader(name);
	}
	const qsf::text& qsf::framework2::get_text(const std::string& name) const {
		return qsf::get_text(name);
	}

	void qsf::framework2::create() {
		if (!this->is_created()) {
	...
	bool qsf::framework2::is_open() const {
		return this->window.isOpen();
	}
	bool qsf::framework2::is_created() const {
		return this->m_created;
	}
	void qsf::framework2::set_info(const std::string& title, qpl::vector2u dimension, qpl::u32 style) {
		this->set_title(title);
		this->set_dimension(dimension);
		this->set_style(style);
	}
	void qsf::framework2::set_title(const std::string& title) {
		this->m_title = title;
		if (this->m_created) {
			this->window.setTitle(title);
		}
	}
	void qsf::framework2::set_dimension(qpl::vector2u dimension) {
		this->m_dimension = dimension;
		this->event.m_screen_dimension = dimension;
	}
	void qsf::framework2::set_style(qpl::u32 style) {
		this->m_style = style;
	}
	void qsf::framework2::hide_cursor() {
		this->window.setMouseCursorVisible(false);
		//todo
	}
	void qsf::framework2::set_window_position(qpl::vector2u position) {
		this->window.setPosition(sf::Vector2i(position));
	}
	qpl::vector2u qsf::framework2::get_window_position() const {
		return qpl::vector2u(this->window.getPosition());
	}
	void qsf::framework2::show_cursor() {
		this->window.setMouseCursorVisible(true);
		//todo
	}
	void qsf::framework2::set_cursor_position(qpl::vector2i position) {
		//todo
	}
	...
	void qsf::framework::internal_update() {
		...
		this->event.m_frame_time = this->frametime;
